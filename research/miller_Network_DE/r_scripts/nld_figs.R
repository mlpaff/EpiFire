...
names <- c("theta", "pI1", "pI2", "I1", "I2", "R1", "R2")#, "C1", "C2")
colnames(data) <- names

data$S = 1 - data$I1 - data$I2 - data$R1 - data$R2 
# data$inc_1 = c(data$C1[2:nrow(data)]-data$C1[1:nrow(data)-1], NA)
# data$inc_2 = c(data$C2[2:nrow(data)]-data$C2[1:nrow(data)-1], NA)
...
melt(data,  id.vars=c("index"), measure.vars = c("S", "I1", "I2", "R1", "R2")) %>%
  ...
  theme_bw() %+replace% theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                              ...
                              legend.title = element_blank(), legend.key.size=unit(1.5, "cm") )
dev.off()
